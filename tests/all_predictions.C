...
const TString Setlistall[5] = {
    "PartList_PPB2002.txt", "PartList_PPB2014.txt", "PartList_PPB2014_CBHN.txt",
...
void prediction(
    TString filename = THERMUS +
                       "/share/Thermus/particles/PartList_PPB2014_CBHN.txt",
    Bool_t decayfix = true);
...
void prediction(TString filename, Bool_t decayfix) {
  ...
  set.InputDecays(THERMUS +
                  "/share/Thermus/particles"); // here true means the decays are
  ...
  TTMThermalFitBSQ fit(&set, &par,
                       THERMUS + "/share/doc/Thermus/tests/prediction.txt");
  ...
  if (debugMode)
    fit.ListYields();
  if (quantRes)
    fprintf(fout, "INFO: quantum Stat and Resonance Width are included \n");
  else
    fprintf(fout, "INFO: quantum Stat and Resonance Width are not included \n");
  radius = fit.GetParameterSet()->GetRadiusPar()->GetValue();
  volume = (4 * 3.1416 / 3.) * radius * radius * radius;
  if (radius && debugMode)
    fprintf(fout, "INFO: radius = %.1f fm, volume = %.2f fm^3\n", radius,
            volume);
  ...
  TTMYield *lYield = 0;
  Int_t idYields[30] = {
      211,         -211, 321,        -321,        310,        2212,
      -2212,       3122, 3312,       -3312,       3334,       -3334,
      313,         333,  3124,       3222,        3224,       3214,
      3114,        3324, 1000010020, -1000010020, 1000020030, 1010010030,
      -1010010030, 411,  421,        4122,        443,        553};
  for (Int_t iYields = 0; iYields < 25; iYields++) {
    lYield = fit.GetYield(idYields[iYields], 0, "PREDICTION");
    fprintf(fout, " %d \t %s \t %6f \n", lYield->GetID1(),
            lYield->GetTMName().Data(), lYield->GetModelValue());
  }

  TTMYield *lRatio = 0;
  lRatio = fit.GetYield(-211, 211, "PREDICTION");
  fprintf(fout, " %d \t %d \t %s \t %6f \n", lRatio->GetID1(), lRatio->GetID2(),
          lRatio->GetTMName().Data(), lRatio->GetModelValue());
  ...
  for (Int_t iRatio = 0; iRatio < 7; iRatio++) {
    ...
    fprintf(fout, " %d \t %d \t %s \t %6f \n", lRatio->GetID1(),
            lRatio->GetID2(), lRatio->GetTMName().Data(),
            lRatio->GetModelValue());
  }
